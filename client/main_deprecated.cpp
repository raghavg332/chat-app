// Earlier version of the client application
        ...
        for (int i = 0; i<2; i++){
            if (fd_list[i].revents & POLLIN){
                if (fd_list[i].fd == STDIN_FILENO){
                    ...
                    std::cout << "\33[2K\r";         // ANSI escape: clear entire line and return cursor to start
                    std::cout << buffer << std::endl;
                    std::cout << std::flush; // Optional: reprint input prompt
